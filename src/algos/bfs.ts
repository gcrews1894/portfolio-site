...
export function bfs(nodes: NodeData[][], start: NodeData, target: NodeData, nodesToAnimate: NodeData[], boardArray: number[][]): [boolean, NodeData[]] {
  const queue: NodeData[] = [start] // Initialize queue with start node
  ...
  visited.add(`${start.row}-${start.col}`) // Mark start node as visited
  ...
  while (queue.length > 0) {
    ...
    for (const neighbor of neighbors) {
      ...
      if (!visited.has(key)) {
        visited.add(key) // Mark neighbor as visited
        cameFrom.set(key, current) // Store the path
